...
loop:
	...
	bne	$t0, $zero, endif1				# If amount of paint is greater than 0, move
	la	$a3, puzzle
...
loop2:
	la	$t0, SOLVE_FLAG
	lw	$t1, 0($t0)
	beq	$t1, $zero, loop2				# if FLAG = true, solve!
	sw	$0, 0($t0)
	...
	j	loop						# check if we have paint

endif1:
	la	$t2, VELOCITY
	li	$t3, 10
	sw	$t2, 0($t2)					# Set velocity to 5
	la	$t0, GET_PAINT_BUCKETS
	lw	$t0, 0($t0)					# t0 = amt of paint buckets you have
	beq	$t0, $zero, loop
	j	endif1
...
bonk_interrupt:
	sw 		$0, BONK_ACK
    #Fill in your code here
	la		$t1, ANGLE	# Turn left if a bonk is called
	li		$t2, 90
	sw		$t2, 0($t1)
	la		$t1, ANGLE_CONTROL
	sw		$zero, 0($t1)
