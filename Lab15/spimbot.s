...
### Put in my data segment
GRIDSIZE = 8
inventory:   .half 0:100
puzzle:      .half 0:164              
heap:        .half 0:4160
...
main:
	...
	li	$t1, 1						# Setting paint to 1
	sw	$t1, SWITCH_MODE($0)

check_for_paint:
	la	$t0, GET_PAINT_BUCKETS
	lw	$t0, 0($t0)					# t0 = amt of paint buckets you have
	li	$t7, 50
	bgt	$t0, $t7, paint					# If amount of paint is greater than 0, paint!
	la	$a3, puzzle
	la	$t1, REQUEST_PUZZLE
	sw	$a3, 0($t1)					# save puzzle pointer to REQUEST_PUZZLE

wait_till_request_acknowledged:
	la	$t0, SOLVE_FLAG
	lw	$t1, 0($t0)
	beq	$t1, $zero, wait_till_request_acknowledged	# if FLAG = true, solve!
	sw	$0, 0($t0)					# set FLAG back to false

# Actually solving the puzzle
	la	$a0, puzzle					# Copying board to heap
	la	$a1, heap
	jal	copy_board
	...
	jal	solve
	la	$t0, puzzle
	sw	$t0, SUBMIT_SOLUTION($0)			# save solution to SUBMIT_SOLUTION
	j	check_for_paint					# check if we have paint

paint:
	la	$t2, VELOCITY
	li	$t3, 10
	sw	$t3, 0($t2)					# Set velocity to 10

	la	$t4, PUP_FLAG
	lw	$t4, 0($t4)
	bne	$t4, $zero, _paint_c

_look_for_pup_loop:
	la	$t2, PICKUP_POWERUP				# Always try to pick up a power up	
	sw	$t1, 0($t2)

	la	$t0, inventory					# Check if we have a pup in our inventory
	sw	$t0, GET_INVENTORY($zero)
	lb	$t1, 8($t0)
	beq	$t1, $zero, _look_for_pup_loop
	li	$t3, 1
	sw	$t3, PUP_FLAG($zero)

_paint_c:
	sb	$zero, USE_POWERUP($zero)			# Try to use first pup
	la	$t0, GET_PAINT_BUCKETS
	lw	$t0, 0($t0)					# t0 = amt of paint buckets you have
	beq	$t0, $zero, check_for_paint
	j	paint

	jr $ra

# =========================================================
# ===========|Puzzle Solve from puzzle_soln.s|=============
# =========================================================
...
bonk_interrupt:
	...
	sw	$t1, SWITCH_MODE($0)
								...
								# Finding either a random angle or turn left
	li		$t2, 90
	la		$t1, PUP_FLAG
	lw		$t1, 0($t1)
	beq		$t1, $zero, set_angle	# Turn left if pup has been received, otherwise turn to a random angle
	la		$t3, TIMER
	lw		$t3, 0($t3)
	rem		$t2, $t3, $t2
...
set_angle:
	...
	la 		$t3, OTHER_BOT_X # Get opponent x
	li		$t4, -1 # the x size of the board, from piazza
	lw		$t3, 0($t3) # Found the bug
	bne $t3, $t4, set_pup_flag_1
	
	j       interrupt_dispatch    				# see if other interrupts are waiting

set_pup_flag_1: # set pup flag to 1 if we have another opponent. We need to move
	li		$t3, 1
	sw		$t3, PUP_FLAG($zero)
	j       interrupt_dispatch    				# see if other interrupts are waiting
...
request_puzzle_interrupt:
	...
	la	$t7, SOLVE_FLAG		
	li	$t6, 1
	sw	$t6, 0($t7)		# Set SOLVE_FLAG to true
